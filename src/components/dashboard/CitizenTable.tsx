...
export function CitizenTable({ data, onRowClick, onExport, onDelete, onBatchDelete, onBatchReOcr }: CitizenTableProps) {
  ...
  const handleDelete = (e: React.MouseEvent, id: number) => {
    ...
    confirm(
      'Xóa hồ sơ',
      'Bạn có chắc chắn muốn xoá hồ sơ này? Hành động này không thể hoàn tác.',
      () => {
        onDelete(id);
      },
  ...
  const handleBatchDeleteClick = () => {
    ...
    confirm(
      'Xóa hồ sơ hàng loạt',
      `Bạn có chắc chắn muốn xoá ${selectedIds.size} hồ sơ đã chọn? Hành động này không thể hoàn tác.`,
      () => {
        if (onBatchDelete) {
      ...
      { isDestructive: true, confirmText: 'Xóa tất cả' }
    );
  };
